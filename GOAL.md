...
**Next increment:** Record one provider-verified Realtime voice turn in the local operator Workshop, rerun the zero-spend preflight, then—after explicit authorization—run the printed twelve-request command for the first provider-backed Map, six-panel GPT Image 2 batch, and narration set. Grade every panel against the checked-in professional image rubric and record the real timing plus first required revision. In parallel, put the reproducible external-use deck in front of its intended professional audience for a cold `Send`/`Revise` decision. Repeat the clean-start run in the Codex in-app browser when that capability is callable, but do not let the unavailable host surface block product-quality work.
...
- [ ] Repeat the same clean-start inspection in the Codex in-app browser when that host capability is callable; do not substitute the production-browser run for this host-specific proof.
