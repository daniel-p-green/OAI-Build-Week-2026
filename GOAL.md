...
- [ ] **Finish the connected product-quality pass.** Simplify first use and navigation around `Capture → Map → Brief → Create`; make the constrained Map clarify evidence, synthesis, direction, and the recommended path; keep grounding visible across created work; and resolve visible responsive or repository-hygiene blockers without adding breadth.
  ...
  - Every focused created-work view has been audited in the live ChatGPT/Codex browser for its Source trail, refinement path, and primary action. Presentation and Infographic retain editable PowerPoint downloads; Audio Overview retains script editing; Sketch retains editable SVG; Image set retains source-backed selective replacement; Storyboard retains versioned panel editing; and Video returns to the exact approved Storyboard. Video cards and the focused player now rest on the first image from the exact Storyboard version that produced that Video instead of presenting a black frame before playback.
