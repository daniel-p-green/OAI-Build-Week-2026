...
Last updated: 2026-07-16 01:37 CT
...
- The authorized session made 42 OpenAI request attempts in total: 18 benchmark requests, two schema-rejected Map attempts, one successful Terra Map, six successful GPT Image 2 panels, and 15 Cedar narration attempts across streaming-WAV validation and timing repair. Exact dollar debit is not returned by these provider responses; observed request shape remains safely below the authorized $50 ceiling, and no additional provider spend was used for UI, Map-layout, or Video Version 2 repairs.
...
- Still open and not claimed: a provider-backed Realtime microphone conversation/interruption/write loop, a provider-generated standalone Audio Overview, the founder's actual brainstorm recording, external professional review, final public submission film/links, and `/feedback`.
...
**Known host risk:** Native Codex/ChatGPT task and voice-turn persistence remain unproven and must not be claimed, but that host sync is no longer the product path. WorkshopLM now owns browser Conversation. The server-minted ephemeral session route, browser WebRTC control, input and assistant transcript reducers, remote audio playback, durable Source/turn boundary, provider event provenance, official function-tool session schema, shared server executor, provider event adapters, and visible Conversation activity are implemented and tested foundations. Capture-only mode remains available for Add Source. Center-column conversation mode now requests audio output, automatic canonical tools, VAD-created responses, and interruption, then saves the provider-spoken answer rather than generating a duplicate local reply. A visibly confirmed write returns its successful function output to the same active voice channel and requests the next response. The whole path must still be live-verified for spoken response, interruption, one read, one visible write, and durable conversation/tool provenance before recording.
...
- [ ] Make voice and text feel like two inputs to the same Conversation, not separate modes. Either path should produce a durable Source, grounded Map updates, and one clear next action without asking the professional to understand models, retrieval, or orchestration.
...
- [ ] The first real GPT Image 2 batch contains deck-usable diagrams, concept visuals, or section art—not generic illustration—and passes a six-panel coherence review.
  - 2026-07-15 zero-spend readiness: the six planned panels are no longer hardcoded demo scenes. Each is derived from the approved Brief, assigned to the strongest matching evidence, and retains its exact claim/source edge through generation, retry fingerprints, the image manifest, and per-image submission assets. Hero concept, systems diagram, evidence chain, decision visual, storyboard sequence, and section art share one reference hash and explicit professional review rubric. `pnpm demo:live` reports a valid six-panel plan with zero issues and `paidCallsMade: false`; the real-generation and human coherence gate remains open.
...
- [x] Activate and verify the designed capture-only foundation for the unproven native task/voice synchronization spike; live speech-to-speech Conversation and tool use remain separately unverified and open below.
...
- [ ] Upgrade and verify the current capture-only `gpt-realtime-2.1` path as WorkshopLM's center-column speech-to-speech voice agent: natural turn taking, barge-in, spoken responses, live transcript, and typed Workshop tool calls over the same durable Conversation/Source contract.
  ...
  - The server-minted ephemeral credential route, direct browser WebRTC session, capture-only VAD configuration, final transcript reducer, private Source persistence, and provider item/event provenance are implemented and tested foundations. Keep this open until an inspected live microphone conversation proves spoken response, interruption, one safe read tool, one visible write tool, and durable transcript/tool provenance.
...
- [ ] Add WorkshopLM's grounded browser text Conversation using Responses streaming and durable Conversation state. Persist local turns, retrieval edges, tool calls, tool results, and OpenAI continuation identifiers without making hosted state the sole source of truth.
...
- [ ] Create a source-traceable Audio Overview as a first-class versioned Production Output: structured grounded script, editable review, `gpt-4o-mini-tts` speech generation, streaming/local playback, standard download, duration, source coverage, model/voice metadata, file hash, and explicit AI-voice disclosure.
