...
- [ ] **Finish the connected product-quality pass.** Simplify first use and navigation around `Capture → Map → Brief → Create`; make the constrained Map clarify evidence, synthesis, direction, and the recommended path; keep grounding visible across created work; and resolve visible responsive or repository-hygiene blockers without adding breadth.
  - Current implementation evidence covers the complete four-stage spine, simplified first use, a live `Build my Map` handoff to GPT-5.6 Terra, a constrained semantic Map that visibly separates Evidence, Synthesis, Direction, and the recommended path, graceful source-backed fallback organization, visible source-link depth on created work, a professional semantic Infographic verified in HTML and editable PowerPoint across desktop and compact layouts, aligned design/public/plugin contracts, responsive browser verification, and a clean ignored-cache boundary verified by a from-scratch check. Founder visual acceptance remains the completion gate.
