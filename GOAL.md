...
Last updated: 2026-07-15 03:23 CT
...
**Current gate:** The P0 screen-composition and canonical-language rebuild, bounded Realtime capture UI, constrained semantic Excalidraw Map, version-aware Outputs history, contextual original-brainstorm reveal, usable Style selection, and completed Storyboard-to-Video handoff are implemented. The root design system is reconciled to that exact no-tabs interface and now has a regression test against legacy vocabulary and actions. Style now lets a professional start from a public website or exact manual brand rules, choose Client pitch, Board presentation, or Team workshop, and preserve the selected intent in the versioned `DESIGN.md` tokens; secondary brand details remain collapsed until requested. While a local render is queued or rendering, the browser silently refreshes the Workshop; once it succeeds, `View video` replaces the obsolete creation action and the real local MP4 appears in Outputs and a responsive focused player. The Map persists direct movement, resizing, and text edits through typed graph history, exposes source trace, and restores each operation through WorkshopLM Undo; its native Excalidraw product chrome is suppressed so the official Apps in ChatGPT shell remains the only surrounding interface system. Outputs now preserve real previews and version history with type, freshness, and source coverage while keeping creation in the single header action. Focused Outputs preserve the artifact-specific source count, title, type, and version; `Show source` is the one dominant action and the external action names the file or video it opens. The focused Video exposes one `Show original` action that keeps the finished work visible beside an honestly labeled transcript or sanitized excerpt and its five resulting Outputs. Five independent cold review passes now identify the current object, source scope, and next action on Map, Brief, Outputs, Storyboard, Sources, and focused Outputs at the verified widths with no critical navigation failure. Judge-facing drafts use the canonical product language and an evidence-gated claim ledger, but final publication remains open until the recorded assets and links exist. The remaining visual completion gate is provider-backed GPT Image 2 gallery media. The Realtime transport is ready for its first provider-backed microphone turn but is not yet live-verified.
...
- [x] Create a bounded live operator path on sample sources for recording the real demo seam.
  - 2026-07-15: `pnpm demo:live` passed its isolated no-spend preflight with two approvals, two traced outputs, one planned GPT-5.6 request, six planned GPT Image 2 requests, five planned TTS requests, and zero provider calls. Live execution now requires `WORKSHOPLM_MAX_PAID_REQUESTS=12`; the separate nine-request routing benchmark requires its own ceiling. Paid execution remains unrun.
...
- [ ] Include real UI evidence and the raw-transcript reveal. The production UI now implements and captures the responsive `Show original` before/after surface with fixture evidence; close only after the dated contemporaneous founder brainstorm and final recorded footage replace that fixture.
