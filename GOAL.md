...
Last updated: 2026-07-15 19:51 CT

## Status

**Active phase:** Three-column workbench rebuild, professional send-it quality, and live demo proof
...
**Next increment:** Rebuild the user-facing product as one responsive, NotebookLM-like three-column WorkshopLM workbench—persistent Sources on the left, WorkshopLM Conversation and the current work object in the center, and Studio/Outputs progress on the right—while staying entirely grounded in the verified Apps in ChatGPT Figma library and documented WorkshopLM composites. WorkshopLM, not Codex, owns the primary chat interface. Preserve the current Map, approvals, source trace, Style, stale-state, and immutable Output contracts. In parallel, record one provider-verified Realtime voice turn, keep provider spend behind explicit authorization, and obtain the cold professional `Send`/`Revise` decision on the external-use deck.
...
1. **One three-column Workshop.** Desktop and wide tablet use one stable `Sources | Conversation and current work | Studio` shell. Columns may resize or collapse, but their meaning and location do not change during the core flow.
...
4. **Studio accumulates finished work.** The right column persistently exposes Brief, Style, Presentation, Infographic, Image set, Storyboard, and Video as an evolving production history with honest empty, planned, in-progress, review, current, failed, and needs-update states.
5. **The current object owns the center.** Conversation and the selected work object share the primary region. Focused review expands only when the Brief, Storyboard, or finished Output needs more space; Sources and Studio remain available in compact or collapsible form.
...
7. **Two consequential approvals.** `Approve brief` and `Approve storyboard` remain the only blocking approvals. Their prerequisites, consequence, and resulting state are visible in the center and Studio column; video creation remains unavailable until Storyboard approval.
...
- [ ] Rebuild WorkshopLM as one responsive three-column workbench with persistent/collapsible Sources on the left, WorkshopLM Conversation plus the selected work object in the center, and persistent/collapsible Studio/Outputs progress on the right.
...
- [ ] Make Capture -> Shape -> Deliver an interactive progress model with completed prerequisites, blockers, and one explicit next action; remove workflow-engine responsibility from the swapping header action.
...
- [ ] Make Studio an accumulating production rail from first use through Video, with visible locked, empty, planned, in-progress, review, current, failed, and needs-update states plus bounded retry.
- [ ] Document every new shell, Conversation, rail, collapse, splitter, inspector, and responsive pattern as an Apps-in-ChatGPT-Figma-derived WorkshopLM composite before implementation; add each to the component inventory, reusable UI layer, and conformance allowlist.
- [ ] Adapt the three-column model for mobile with a sticky Sources / Work / Studio object-stage switcher; preserve source scope, production state, and the next action without squeezing desktop columns or hiding critical capability.
- [ ] Verify the rebuilt path at 1200x800, 1024x768, 390x844, keyboard-only, 200% logical zoom, reduced motion, and WCAG AA. Preserve all deterministic seam, approval, stale-version, source-trace, render, and submission checks.
