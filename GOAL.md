...
Last updated: 2026-07-15 20:35 CT
...
**Next increment:** Define one shared typed Workshop tool registry for text and voice, then upgrade the grounded Conversation from its verified local retrieval foundation to Responses streaming and speech-to-speech Realtime behavior with durable provider/tool provenance. Provider execution remains behind explicit authorization. In parallel, dogfood the hero workflow with one external-use, editable, source-defensible deck and obtain a cold `Send`/`Revise` decision.
**Stable-workbench evidence:** Phase 2 is implemented and verified: Sources remain visible on the left, durable grounded Conversation or the current work object stays centered, Production remains visible on the right, and mobile retains direct access to Chat, Map, Brief, Outputs, and Storyboard. Text questions remain queries rather than silently becoming Sources; answers persist retrieval edges and open the exact cited excerpt. Voice capture shares the same surface and explicitly becomes a private Source. Collapsible rails, affected-work disclosure, Responses streaming, and speech-to-speech Realtime tool use remain open.
...
**Known host risk:** Native Codex/ChatGPT task and voice-turn persistence remain unproven and must not be claimed, but that host sync is no longer the product path. WorkshopLM now owns browser Conversation. The server-minted ephemeral session route, browser WebRTC control, final-transcript reducer, durable Source boundary, and provider event provenance are implemented and tested foundations; the path must be upgraded from capture-only mode and live-verified for spoken response, interruption, tool use, and durable conversation provenance before recording.
...
- [ ] Add WorkshopLM's grounded browser text Conversation using Responses streaming and durable Conversation state. Persist local turns, retrieval edges, tool calls, tool results, and OpenAI continuation identifiers without making hosted state the sole source of truth.
  - 2026-07-15 local foundation: durable turns, BM25 retrieval, exact evidence edges, a typed `source_search` operation, and responsive Conversation UI are implemented and verified. Keep this open until Responses streaming, tool-result persistence, and continuation identifiers are provider-verified.
- [ ] Expose one shared typed Workshop tool registry to text Responses and Realtime voice. Voice and text must obey the same source scope, approval, stale-state, privacy, and confirmation rules.
