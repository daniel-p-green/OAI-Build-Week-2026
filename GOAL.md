...
Last updated: 2026-07-15 21:30 CT
...
**Known host risk:** Native Codex/ChatGPT task and voice-turn persistence remain unproven and must not be claimed, but that host sync is no longer the product path. WorkshopLM now owns browser Conversation. The server-minted ephemeral session route, browser WebRTC control, final-transcript reducer, durable Source boundary, provider event provenance, official function-tool session schema, shared server executor, provider event adapters, and visible Conversation activity are implemented and tested foundations. Capture-only mode still sets `tool_choice: none`; the path must enable the confirmation-aware voice loop and speech output, then be live-verified for spoken response, interruption, one read, one visible write, and durable conversation/tool provenance before recording.
