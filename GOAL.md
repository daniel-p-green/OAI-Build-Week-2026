...
Last updated: 2026-07-15 20:15 CT
...
**Next increment:** Add WorkshopLM's own grounded Conversation to the stable three-column workbench, with one text composer and integrated Realtime voice that operate the current Sources and work object. Then dogfood the hero workflow against the professional replacement test: create one external-use, editable, source-defensible deck and obtain a cold `Send`/`Revise` decision. Keep provider spend behind explicit authorization.
**Stable-workbench evidence:** Phase 1 is implemented and verified: Sources remain visible on the left, the current work object stays centered, Production remains visible on the right, the header no longer swaps through workflow actions, and mobile retains direct access to Map, Brief, Outputs, and Storyboard. Grounded in-product Conversation, collapsible rails, and the contextual evidence/affected-work inspector remain open.
...
11. **Mobile adapts rather than shrinks.** Narrow screens use one column plus a sticky object/stage switcher for Sources, Conversation/Map, and Studio. No critical function disappears, and the current scope, stage, and next action remain reachable without reproducing three squeezed columns.
...
- Preserve NotebookLM's familiar Sources -> central Conversation/work -> Studio mental model inside one WorkshopLM application. The stable three-column geography is a locked product decision, not a temporary desktop treatment.
...
- **Studio** is the persistent user-facing production column. **Outputs** is the creation and history collection inside Studio; an **Output type** is a guided creation path; an **Output** is one professional deliverable; an **Output set** is a user-selected group created from the same approved state.
...
- [ ] Add WorkshopLM's own grounded text composer to the center column and integrate the existing Realtime voice capture there. Persist each completed turn under the existing transcript/Source contract; do not depend on Codex chat state for the user workflow.
...
- [x] Document every implemented shell, rail, and responsive pattern as an Apps-in-ChatGPT-Figma-derived WorkshopLM composite before implementation; add each to the component inventory, reusable UI layer, and conformance allowlist. Future Conversation, collapse, splitter, and inspector patterns remain subject to the same rule before they ship.
- [x] Adapt the three-column model for mobile with a compact Map / Brief / Outputs / Story object switcher; preserve source count, production state, and the next action without squeezing desktop columns or hiding critical capability.
- [x] Verify the rebuilt Phase 1 path at 1200x800, 1024x768, 390x844, keyboard-only, 200% logical zoom, reduced motion, and WCAG AA. Preserve all deterministic seam, approval, stale-version, source-trace, render, and submission checks. Re-run this gate after Conversation and collapse controls land.
