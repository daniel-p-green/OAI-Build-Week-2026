...
The current phase is **submitted, with the Devpost copy and thumbnail owner-approved but the public demo still rejected**. Daniel explicitly approved and published the revised why-first description, accurate Sol/Terra usage, corrected non-published-plugin boundary, judge fields, and crop-safe thumbnail. The published 2:39 film is not owner-approved: Daniel identified echo, excessive music ducking, low-resolution product footage, and a weak why story. No YouTube, X, further Devpost, release, or other public change may be made without explicit human review and approval. The replacement path now has verified full-frame 1080p interaction footage, twelve native 4K product states, and a local 30-second why-first opening proof; Daniel's review of that exact proof remains the gate before the complete 2:39 render.
...
- [ ] **Finish the connected product-quality pass.** Simplify first use and navigation around `Capture → Map → Brief → Create`; make the constrained Map clarify evidence, synthesis, direction, and the recommended path; keep grounding visible across created work; and resolve visible responsive or repository-hygiene blockers without adding breadth.
  ...
  - The sanitized judge fixture now replays the verified six-image GPT Image 2 set and carries those exact, hash-bound visuals through the Presentation and Storyboard. The Image set remains coherent and fully reviewable at desktop, compact, and mobile widths, including contained status and selective-replacement actions. Its lead role is labeled `Primary visual` instead of the creative-tool jargon `Hero concept`, while the detailed composition prompt and grounded evidence binding remain unchanged. This adds no provider call or spend.
  - The desktop and compact Map now uses the canvas as a constrained reasoning system rather than a generic source diagram: Evidence, Synthesis, and Direction occupy distinct auto-fitted lanes; Source identity is attached quietly to evidence instead of duplicated as large canvas boxes; convergence links recede; and the recommended Direction carries the sole strong visual emphasis. Semantic gutters keep the regions separate, evidence-to-synthesis relationships collapse into one quiet cluster connection, same-layer line noise is suppressed, and a one-Source Map does not repeat the same caption beneath every claim. The six-claim first-use case and the simple judge fixture both remain legible, editable, and source-inspectable. Mobile retains the deliberate Direction → Synthesis → Evidence outline.
  - First-use Map headlines now preserve complete meaning instead of ending in visibly truncated phrases. The deterministic organizer condenses the time-cost evidence into a complete statement, synthesizes it with the exact-source requirement as `Client-ready work needs source traceability`, and presents the complete `Create work a consultant can refine and present` direction in both the canvas and overview. The live GPT-5.6 contract now requires complete plain-language headlines of at most 64 characters and forbids ellipses or incomplete phrases. Mobile proves the same Direction → Synthesis → Evidence hierarchy and keeps Undo as a quiet contextual control rather than an empty full-width bar.
  - Brief approval now promotes a concise, genuinely useful Synthesis when it is stronger than a long source sentence. The first-use Brief reads `Client-ready work needs source traceability`, keeps the complete approved Direction and three exact Source links beneath it, and no longer narrows the Workshop to Presentation as the singular outcome. The full evidence remains visible and inspectable at desktop and mobile widths; Style remains the one contextual next action.
  - The concentrated visible-quality pass is implemented. Desktop Map lanes now form one tighter Evidence → Synthesis → Direction argument instead of spreading a small graph across the field. Compact and mobile headers retain the current position (`Map · 2 of 4`), and mobile Evidence cards show the complete sourced statement rather than a clipped headline. Source inspection reserves a side-by-side reading area at desktop so the originating claim and surrounding work remain visible. Audio Overview now uses direct editorial chapters (`The opportunity`, `Evidence to trust`, `Recommended direction`) without template filler, while Storyboard review labels the current Scene and keeps its exact Source locator beside timing and edit controls. The only remaining product-quality proof gate is the authentic founder-derived Workshop; this does not authorize another product redesign or more output breadth.
  - The Map now opens on the evidence that actually supports its synthesis and direction instead of presenting every extracted statement at equal weight. It shows at most three key evidence items, removes a grounded claim when the same statement is already the recommended Direction, and offers one quiet `Show more` action for the complete editable set. Mobile keeps every non-duplicate item in its Direction → Synthesis → Evidence outline. A fresh empty-Workshop run rendered two key evidence items, the specific synthesis `Client-ready work needs source traceability`, and one actionable Direction; expanding revealed the two secondary findings without changing the hierarchy or adding a whiteboard mode.
  - Mobile Map review now begins directly with the Direction object, followed by Synthesis and Evidence. The redundant overview strip that repeated only `Recommended direction` above the same card has been removed at phone width; it returns only as a contextual Undo surface after a real Map edit. Desktop and compact retain the useful evidence counts, recommended-direction summary, and constrained Excalidraw canvas.
  - The Infographic now preserves editorial hierarchy without allowing Source citations to collide with headings or supporting text. Its focused preview is explicitly rendered and inspected at desktop, compact, and mobile widths; every card must prove that its content and citation fit before the visual regression path passes. The editable one-slide PowerPoint path remains unchanged.
  - The Audio Overview now opens as a professional listening experience rather than a three-column edit form. Playback, title, duration, Cedar disclosure, chapter structure, and Source actions remain visible at desktop and mobile widths; narration is readable by default, while script editing is a deliberate secondary action with cancel and versioned save behavior. The current grounded provider artifact is a hash-bound 29.25-second Cedar recording, and the deterministic browser fixture replays those exact bytes instead of a misleading placeholder.
  - Website-derived Style now opens as a compact visual-system decision rather than a long setup form. The professional sees the source website, intended use, palette, typography fallback, findings, brand-asset count, and one primary `Use this style` action; exact colors, fonts, licensing confirmation, assets, and negative rules remain available through `Edit details`. The complete path is rendered and verified at desktop, compact, and mobile widths without weakening the versioned `DESIGN.md` contract.
  - Create now opens immediately instead of leaving the professional on the Brief during a multi-format run. One quiet status names the current format and reports bounded progress while finished work appears progressively and remains usable. First-run guidance and update completion notices no longer duplicate each other; failure still preserves every completed format and points to the remaining work.
  - Repository roles and proof boundaries are now explicit in `docs/REPOSITORY-MAP.md`. The normal `pnpm check` fails if runtime databases, caches, or test results become tracked; if active-document links or privacy-critical ignore rules break; or if a newly tracked file crosses the 10 MiB review threshold. Existing evidence media remains intentional, and current user-owned preview/design-audit work remains untouched.
  - Capture now treats voice, notes, websites, and local PDFs as one simple starting surface. A professional can choose a PDF through the approved component system instead of typing an absolute filesystem path; the original stays under the local Workshop data root, extracted claims remain private by default, and the existing exact-grounding path is unchanged. Conversation and Source management remain inside Capture rather than becoming new navigation stages.
  - The four-stage status model now treats Source coverage as part of Map freshness. Adding or changing selected Sources marks Map and Brief `Needs update`, replaces premature approval with one `Update Map` action, refreshes the constrained Evidence → Synthesis → Direction canvas through the existing live-or-deterministic Map path, and blocks approval at the service boundary until a prior Brief's Map covers the current Source claims. The professional never has to reason about claim IDs or retrieval state.
  - The deterministic Map now promotes the strongest source-backed recommendation into a concise action direction instead of inserting a generic instruction to approve the Brief. Before approval it is labeled `Recommended direction`; afterward the same reviewed idea is labeled `Approved direction`, so a completed Workshop no longer contradicts its current status or next action. The full source statement remains attached as the node body and evidence locator.
  - A genuinely empty, isolated Workshop now passes the first Source-to-Map interaction without fixture setup: pasted meeting context becomes two evidence items, one synthesis, and a direct proposed decision in one `Build my Map` action. Direction ranking distinguishes an actual recommendation (`the team should prioritize…`) from surrounding prose that merely mentions recommendations, and the resulting Direction-first hierarchy remains legible at desktop and mobile widths. Independent human use remains a separate proof gate.
  - Brief approval now preserves the exact professional direction reviewed on the Map instead of re-ranking the surrounding evidence and silently changing the decision. The direction is not duplicated as evidence. A first-time professional without a saved Style can choose a clean professional default in one action, while website-derived and manual Style remain available; Create becomes the single next action immediately afterward.
  - Deterministic Synthesis is now actual Workshop content rather than the placeholder `What the Sources show`: it selects the strongest non-direction findings, writes their complete meaning into the derived node, and gives the canvas a specific professional headline. Brief approval selects a grounded Outcome distinct from the approved Direction and, when available, lists only separate supporting evidence. The fresh leadership scenario now reads as concise launch need → two-week pilot decision → source-traceability evidence at desktop and mobile widths.
  - Every focused created-work view has been audited in the live ChatGPT/Codex browser for its Source trail, refinement path, and primary action. Presentation and Infographic retain editable PowerPoint downloads; Audio Overview retains script editing; Sketch retains editable SVG; Image set retains source-backed selective replacement; Storyboard retains versioned panel editing; and Video returns to the exact approved Storyboard. Focused Presentation, Infographic, and Sketch views now give their editable export the sole primary emphasis while keeping the Workshop-level Storyboard review visible but secondary. Storyboard itself is review-first: sequence, selected visual, narration, timing, and Source access remain visible, while the full title/narration/timing form appears only after `Edit panel` and can be cancelled without changing the reviewed version. Video cards and the focused player now rest on the first image from the exact Storyboard version that produced that Video instead of presenting a black frame before playback.
  - The approved Brief now keeps the complete active Style name visible at compact desktop widths, with palette and edit controls moving to a quiet second line instead of compressing the governing visual identity into an ellipsis. A geometric browser assertion protects this five-second context alongside the refreshed 1024px visual baseline.
  - Created work now presents every format as a peer instead of visually or behaviorally positioning Presentation as the product. The gallery no longer assigns a Presentation-only hero class or makes first-run orientation depend on Presentation existing; its browser contract selects formats by their human labels and verifies equal Presentation and Infographic card width. The canonical completed fixture shows Presentation, Infographic, Audio Overview, Sketch, Image set, and Storyboard together; the approved state adds Video as the seventh connected expression. Four formats are immediately discoverable at desktop and compact widths, while the complete pre-Video family fits in the first mobile screen with Source coverage and freshness still visible. Focused object views retain the large review surface after selection.
  - The final format is labeled simply `Video` throughout the professional interface: Created work, focused review, Workshop index, history, responsive headers, and Source-backed original reveal. `Demo video` is retired from active product copy; fixture filenames may retain historical implementation names without leaking them into the experience.
  - A fresh one-Source leadership Workshop now preserves one reviewed recommendation across the Brief, Presentation, Infographic, Audio Overview decision chapter, Decision visual, and final Storyboard panel. Decimal evidence such as `2.4 hours` remains one grounded claim; Storyboard titles are unique and end on the approved Direction; and Excalidraw line wrapping no longer turns a read-only Map visit into a false `Needs update` cascade. The live in-app-browser run remained current after reopening the Map and exposed source links and refinement paths across all six created formats. The fresh-product audit itself used no provider request; one bounded Cedar TTS request then refreshed the judge fixture so its spoken decision matches the current approved Brief.
  - The judge-facing narrative now matches the implemented interaction precisely: Source trust is described through the visible `Show source` link rather than a nonexistent click-any-statement gesture, the public Video requirement is explicitly under three minutes, and the current verification counts are reconciled. A fresh deterministic acceptance package verifies as a valid, current, untampered 25-asset `partial` set. Final third-party-asset authorization remains open until the founder-derived public package exists and can be inventoried directly.
...
- [x] **Review and refine the resulting knowledge work.** The exact 30-asset package, six-image contact sheet, twelve-beat product capture, improved Product Story Site, ten-scene film sheet, and seven-world output-flex sheet were inspected together for source trust, visual coherence, legibility, licensing, and professional usefulness. The resulting package and film manifests are ready, valid/current/untampered, and record no limitations.
- [ ] **Complete and review the public video master.** The current local-only review file is `outputs/demo-film-local-review-v5/workshoplm-demo-review-v5.mp4`: a 158.6-second product-first HyperFrames cut built from the real 100.84-second workflow capture, Screen Studio-style focus moves, restrained beat sweeps, an animated finished-output montage, and one short Codex/GPT-5.6 proof composition. It uses five consistent Cedar passages from `gpt-4o-mini-tts-2025-12-15` and the exact authorized `Different Window (DOLBY).wav` stereo master with manual gain rides only—no stem separation, filtering, sidechain, pitch change, time stretch, reverb, delay, chorus, or narration doubling. HyperFrames lint, runtime check, and inspect passed; the MP4 decodes cleanly, measures -14.59 LUFS with -1.50 dB true peak, and its mixed-export transcription recovers the complete required narration. This remains a private review candidate; Daniel's visual and listening approval is mandatory before any canonical or public replacement.
- [x] **Verify and submit.** Devpost submission `1108961` reports `Submitted`. On July 21 Daniel approved a revised tagline and why-first description, accurate GPT-5.6 Sol orchestration and Terra runtime wording, removal of the unsupported published-plugin claim, no-credential judge instructions, the majority-core `/feedback` ID, and the crop-safe 3000×2000 thumbnail. Live read-back returned the revised copy, and the public project page references the processed thumbnail CDN asset. The public YouTube upload at `https://youtu.be/gwi_q6X1i5g` remains playable but owner-rejected and must be replaced only after human review. The earlier X announcement and portable release tag remain unchanged.
