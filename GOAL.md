...
Last updated: 2026-07-15 20:49 CT
...
**Known host risk:** Native Codex/ChatGPT task and voice-turn persistence remain unproven and must not be claimed, but that host sync is no longer the product path. WorkshopLM now owns browser Conversation. The server-minted ephemeral session route, browser WebRTC control, final-transcript reducer, durable Source boundary, provider event provenance, and official function-tool session schema are implemented and tested foundations. Capture-only mode sets `tool_choice: none`; the path must gain a server-side executor, enable speech output, and be live-verified for spoken response, interruption, one read, one visible write, and durable conversation/tool provenance before recording.
...
- [ ] Add WorkshopLM's grounded browser text Conversation using Responses streaming and durable Conversation state. Persist local turns, retrieval edges, tool calls, tool results, and OpenAI continuation identifiers without making hosted state the sole source of truth.
  - 2026-07-15 local foundation: durable turns, BM25 retrieval, exact evidence edges, a typed canonical `search` operation, and responsive Conversation UI are implemented and verified. Keep this open until Responses streaming, tool-result persistence, and continuation identifiers are provider-verified.
