...
The current phase is **submitted and frozen for OpenAI Build Week**. The public repository now presents the runnable product and deterministic judge path without publication packages, raw Source media, research screenshots, or local review exports. Generated evidence and submission history remain recoverable from Git history but are no longer part of the current public tree. No further Devpost, YouTube, release, or repository publication change is authorized without explicit owner direction.
...
- [x] **Freeze the submitted public experience.** The Devpost entry, public Video, public repository, and portable release are the submitted Build Week record. Post-submission review experiments are local-only and outside the active product tree.
  - A post-freeze worktree audit retained the newer mainline product and demo-video proof rather than overlaying stale, conflict-heavy UI experiments. Two independently useful hardening changes were carried forward: generated artifact responses are sandboxed and never cached, and the Workshop JSON endpoint accepts only an explicit action allowlist with PDF ingestion limited to the bounded multipart upload path.
- [x] **Verify and submit.** Devpost submission `1108961` reports `Submitted`. Live read-back confirmed the approved project copy, judge instructions, required `/feedback` task ID, public Video, repository, and crop-safe thumbnail.
...
- **Current responsive UI proof:** production-browser regression baselines in [`apps/web/tests/visual/__screenshots__/`](apps/web/tests/visual/__screenshots__/)
- **Deterministic judge proof:** the sanitized fixtures in [`fixtures/`](fixtures/) and executable `pnpm demo:e2e` path
- **Submission rules record:** [`research/hackathon/SUBMISSION-CHECKLIST.md`](research/hackathon/SUBMISSION-CHECKLIST.md)
