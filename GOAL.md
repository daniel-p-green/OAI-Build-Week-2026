...
The current phase is **submitted and publicly launched**. The core product, founder-authorized connected Workshop, improved Product Story Site, 2:39 beat-synchronized public demo, portable judge release, public YouTube playback, Devpost submission, and X announcement are verified. The earlier 2:20 film was explicitly rejected and is preserved only as historical evidence.
...
- [x] **Complete and review the public video master.** The canonical master is now the 158.6-second why-first HyperFrames cut at `outputs/demo-film-final/workshoplm-demo.mp4`. It uses the full Daniel-provided `Different Window` master as its edit skeleton, lands every major section and four-beat output cut on the detected grid with 0 ms measured drift, includes founder-authorized live-workflow motion, and uses disclosed Cedar narration. Full decode passed; the mixed export measures -14.6 LUFS integrated, 10.5 LU loudness range, and -1.6 dB true peak. A full `gpt-4o-mini-transcribe` pass recovered the complete ten-section story, including Codex, GPT-5.6 Terra, both approvals, and HyperFrames local rendering.
