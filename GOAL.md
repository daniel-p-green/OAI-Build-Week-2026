...
Last updated: 2026-07-16 07:01 CT
...
- The canonical fixture recording now follows the current capture-first onboarding and native workbench instead of the retired pre-onboarding selectors. Its 41.36-second, 12-beat run visibly proves the Sources rail, exact Source reveal, Map edit, Brief approval, Style choice, current-only Outputs gallery, Storyboard edit and approval, local render, and six-Output original-brainstorm reveal. The 2:20 editorial rough cut was rebuilt from this footage with the already verified ten-clip OpenAI Cedar narration; the default build now reuses that verified manifest instead of silently downgrading to a local guide voice.
...
- **Professional promise:** From your meetings and documents to a deck you can defend, with every claim traced to its source.
- **Primary user:** A consultant, strategist, or enablement lead producing client-facing or leadership-facing work every week.
- **Wedge deliverable:** The grounded, branded, source-defensible deck. Infographic, images, Storyboard, and Video support that hero workflow.
...
- [x] Create a source-traceable Audio Overview as a first-class versioned Production Output: structured grounded script, editable review, `gpt-4o-mini-tts` speech generation, streaming/local playback, standard download, duration, source coverage, model/voice metadata, file hash, and explicit AI-voice disclosure.
  ...
  - 2026-07-15 implementation and local proof: approved Brief/Style now create a three-part grounded script with exact claim/source/chunk/locator edges; edits create an immutable new version while retaining those edges; Map, Source scope, and Style changes mark prior versions `Needs update`. Audio Overview appears in the Outputs gallery and Production rail, opens a focused official-component review surface, reveals the exact Source per section, exposes AI-voice disclosure, and provides local playback plus standard download with duration, byte count, SHA-256, model, voice, instructions, and request ID provenance. The shared Responses/Realtime/plugin tool registry can create and trace it. The spend-gated Speech adapter validates a real WAV before persistence and retains the reviewed script on provider failure. Deterministic adapter tests and a local WAV fixture prove the complete UI/storage/recovery seam; keep this item open until one authorized `gpt-4o-mini-tts` response is generated and audibly inspected.
...
- [ ] Make the public video the canonical judge path: show the live seam, one Codex plugin doorway moment, both approval gates, editable control, provenance, and the raw brainstorm → finished submission reveal in under three minutes. The repeatable capture now proves the two gates, semantic edit, source trace, local render, and reveal as one 12-beat sequence. The evidence-gated edit targets 2:20 across ten shots and fail-closes on every missing provider, founder, host, Codex, or final-export input; it intentionally remains a draft until those artifacts exist.
  ...
  - 2026-07-16 current-workbench recapture: the automated 12-beat fixture recorder was reconciled to the capture-first onboarding, persistent desktop Sources rail, current Style sheet, current-only Outputs gallery, exact object navigation, queued-render status, and six-Output reveal. The successful 41.36-second VP8 capture reached `briefApproved`, `storyboardApproved`, and `videoState: rendered`; its rebuilt 2:20 H.264/AAC rough cut uses all ten verified `gpt-4o-mini-tts` Cedar clips. Visual inspection accepted the refreshed contact sheet and the Source, Storyboard, and reveal frames. The film verifier still reports exactly the two honest blocked shots: founder capture and final meta-reveal.
...
- [Current provider-backed editorial rough cut](outputs/demo-film-rough-cut/workshoplm-demo-rough-cut.mp4)
