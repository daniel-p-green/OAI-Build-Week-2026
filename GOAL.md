...
The current phase is **final publication**. The core product, founder-authorized connected Workshop, improved Product Story Site, 2:39 beat-synchronized public-demo master, and populated Devpost draft are verified. The earlier 2:20 film was explicitly rejected and is preserved only as historical evidence. The remaining external gate is a public YouTube upload and its logged-out playback URL; that URL then unlocks the final Devpost submission and X post. A new stable judge release must bind the 2:39 master before submission because the July 20 tag still points to the rejected edit.
...
- [ ] **Verify and submit.** The local final-film verifier and submission-packet verifier pass against the 2:39 master, and the populated Devpost draft remains at `https://devpost.com/software/workshoplm`. Remaining: commit and push a new stable judge release for this master, upload it publicly to YouTube, verify playback logged out, add that URL to Devpost, submit, and publish the prepared X post.
