...
export async function lockWebsiteStyle(rawUrl: string, root?: string, fetchImpl: typeof fetch = fetch): Promise<WorkshopState> {
  ...
  return write({ ...current, style: { version: (current.style?.version ?? 0) + 1, source: "website", name: `${title} foundation`, accent: palette[0] ?? "#1668E3", ink: palette[1] ?? "#171816", paper: palette[2] ?? "#F4F2EC", logos: [], licensedFonts: [], references: [url.toString()], negativeRules: [], intentProfile: "client_facing_pitch", referenceUrl: url.toString(), lockedAt: updatedAt, stale: false }, visualDna: current.visualDna ? { ...current.visualDna, stale: true, approved: false } : undefined, storyboardApproved: false, videoState: "blocked", updatedAt }, root);
}
...
export function lockManualStyle(input: ManualStyleInput = {}, root?: string): WorkshopState {
  ...
  return write({ ...current, style: { version: (current.style?.version ?? 0) + 1, source: "manual", name: input.name?.trim() || "Editorial thinking instrument", accent: color(input.accent, "#1668E3"), ink: color(input.ink, "#171816"), paper: color(input.paper, "#F4F2EC"), logos: cleanStyleList(input.logos), licensedFonts: cleanStyleList(input.licensedFonts), references: cleanStyleList(input.references), negativeRules: cleanStyleList(input.negativeRules), intentProfile, lockedAt: updatedAt, stale: false }, visualDna: current.visualDna ? { ...current.visualDna, stale: true, approved: false } : undefined, storyboardApproved: false, videoState: "blocked", updatedAt }, root);
}
