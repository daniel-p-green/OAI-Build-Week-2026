...
function assertBuildable(state: WorkshopState, root: string): SubmissionInputSnapshot {
  ...
  if (!state.outputs.some((output) => output.type === "deck" && !output.stale) || !state.outputs.some((output) => output.type === "infographic" && !output.stale)) throw new Error("Submission packaging requires current Slides and Infographic Outputs.");
...
export async function buildSubmissionOutputSet(root: string, options: BuildSubmissionOptions = {}): Promise<{ manifestPath: string; outputSet: SubmissionOutputSetType }> {
  ...
  const deckName = `slides${extname(deck.relativePath) || ".html"}`;
  const editableDeckName = deck.editableRelativePath ? "slides.pptx" : undefined;
