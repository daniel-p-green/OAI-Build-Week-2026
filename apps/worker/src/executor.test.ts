...
describe("worker executor", () => { it("fails queued video honestly when its local renderer fails", async () => { const root = await mkdtemp(join(tmpdir(), "workshop-executor-")); applyWorkshopAction("approveBrief", root); applyWorkshopAction("lockManualStyle", root); applyWorkshopAction("approveStoryboard", root); applyWorkshopAction("renderVideo", root); const result = await executeOne(root, async () => { throw new Error("renderer unavailable"); }); expect(result.state).toBe("failed"); expect(readWorkshopState(root).videoState).toBe("queued"); await rm(root, { recursive: true, force: true }); }); });
