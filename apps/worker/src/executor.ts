...
export function buildWorkshopVideoHtml(state: WorkshopState): string {
  ...
  let start = 0;
  const scenes = state.storyboard.panels.map((panel, index) => { const scene = `<section id="panel-${index + 1}" class="clip panel panel-${index % 3}" data-start="${start}" data-duration="${panel.durationSeconds}" data-track-index="1"><div class="eyebrow">WORKSHOPLM · APPROVED STORYBOARD V${state.storyboard.version}</div><h1>${escapeHtml(panel.title)}</h1><p>${escapeHtml(panel.narration)}</p><div class="disclosure">Audio: deterministic local placeholder tone</div></section><audio id="panel-${index + 1}-audio" class="clip" src="panel-${index + 1}.wav" data-start="${start}" data-duration="${panel.durationSeconds}" data-track-index="2" data-volume="0.12"></audio>`; start += panel.durationSeconds; return scene; }).join("");
