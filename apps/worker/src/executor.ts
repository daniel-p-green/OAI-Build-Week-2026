...
export function buildWorkshopVideoHtml(state: WorkshopState): string {
  ...
  let start = 0;
  const scenes = state.storyboard.panels.map((panel, index) => {
    const image = currentImageForPanel(state, panel);
    const evidence = panel.evidence[0];
    const source = evidence ? state.sourceItems.find((candidate) => candidate.id === evidence.sourceId) : undefined;
    const sourceLabel = [source?.title ?? "Approved source", evidence?.locator].filter(Boolean).join(" · ");
    const scene = `<section id="panel-${index + 1}" class="clip panel panel-${index % 3}${image ? " with-image" : ""}" data-start="${start}" data-duration="${panel.durationSeconds}" data-track-index="1"><div class="scene-content">${image ? `<img class="scene-image" src="panel-${index + 1}.png" alt="">` : ""}<div class="copy"><div class="eyebrow">WORKSHOPLM · SOURCE-GROUNDED</div><h1>${escapeHtml(panel.title)}</h1><p>${escapeHtml(panel.narration)}</p></div><div class="panel-number">${String(index + 1).padStart(2, "0")} / ${String(state.storyboard.panels.length).padStart(2, "0")}</div><div class="source-progress"><span>SOURCE ${String(index + 1).padStart(2, "0")}</span><b>${escapeHtml(sourceLabel)}</b><i><em></em></i></div><div class="disclosure">${disclosure}</div></div></section><audio id="panel-${index + 1}-audio" class="clip" src="panel-${index + 1}.wav" data-start="${start}" data-duration="${panel.durationSeconds}" data-track-index="2" data-volume="0.92"></audio>`;
  ...
  const motion = state.storyboard.panels.map((panel, index) => {
    ...
    const commands = [
      ...
      `tl.fromTo("${id} .source-progress em",{scaleX:0},{scaleX:1,transformOrigin:"left center",duration:${Math.max(0.2, panel.durationSeconds - 0.28)},ease:"none"},${enter + 0.14});`,
...
export async function executeOne(root: string, run: RunCommand = defaultRun) : Promise<ExecuteResult> {
  ...
  try {
    ...
    await copyFile(require.resolve("gsap/dist/gsap.min.js"), join(staging, "gsap.min.js"));
