...
it("builds an asset plan from approved graph, brief, evidence, and Style Foundation versions", async () => { const root = await mkdtemp(join(tmpdir(), "workshop-plan-")); await ingestSource({ title: "Evidence", origin: "Fixture", text: "A plan must name its evidence." }, root); applyWorkshopAction("approveBrief", root); lockManualStyle({ intentProfile: "board_deck" }, root); createVisualDna(root); approveVisualDna(root); const plan = generateAssetPlan(root).assetPlan; expect(plan).toMatchObject({ version: 1, briefVersion: 1, styleVersion: 1, visualDnaVersion: 1, stale: false }); expect(plan?.items.map((item) => item.outputType)).toEqual(["deck", "infographic", "images", "storyboard", "video"]); expect(plan?.items.map((item) => item.title)).toEqual(["Slides", "Infographic", "Image set", "Storyboard", "Demo video"]); expect(plan?.items.map((item) => item.prompt).join(" ")).not.toMatch(/FRAME\.md|Visual DNA|provenance|render/i); expect(plan?.evidenceClaimIds[0]).toMatch(/^claim-/); expect(plan?.items.every((item) => { const evidence = item.evidence[0]; return Boolean(evidence?.claimId && plan.evidenceClaimIds.includes(evidence.claimId) && evidence.sourceId && evidence.chunkId); })).toBe(true); await rm(root, { recursive: true, force: true }); });
...
it("persists an output failure and clears only that recovery state after a successful retry", async () => { const root = await mkdtemp(join(tmpdir(), "workshop-output-recovery-")); applyWorkshopAction("approveBrief", root); lockManualStyle({}, root); const failed = recordOutputFailure("deck", root); expect(failed.outputRecovery?.deck).toMatchObject({ attempts: 1, message: "Slides could not be created. Your Brief and Style are safe." }); const failedAgain = recordOutputFailure("deck", root); expect(failedAgain.outputRecovery?.deck?.attempts).toBe(2); const recovered = await generateOutput("deck", root); expect(recovered.outputRecovery?.deck).toBeUndefined(); expect(recovered.outputs.at(-1)).toMatchObject({ type: "deck", stale: false }); await rm(root, { recursive: true, force: true }); });
