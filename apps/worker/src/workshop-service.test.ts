...
it("keeps grounded claims sentence-complete when a long paragraph crosses the chunk limit", async () => {
  ...
  try {
    ...
    const decimalClaim = "Active users saved 2.4 hours per week.";
    ...
    const state = await ingestSource({ title: "Long meeting", origin: "Pasted notes", text: `${context} ${decimalClaim} ${finalClaim}` }, root);
    ...
    expect(claims).toEqual(expect.arrayContaining([expect.objectContaining({ text: "Active users saved 2.4 hours per week" })]));
...
it("organizes grounded claims into an honest deterministic evidence-to-direction Map without recording an AI run", async () => {
  ...
  delete process.env.WORKSHOPLM_SEEDED_FIXTURE;
  try {
    await ingestSource({ title: "Strategy notes", origin: "Pasted notes", text: "Client work is fragmented across tools.\n\nEvery factual claim needs a source locator.\n\nThe team should approve direction before creating work." }, root);
    ...
    expect(organized.aiRuns).toHaveLength(0);
    expect(organized.graphState).toContain("operation-map-organizer-");
    expect(organizeGroundedMap(root).mapNodes).toEqual(organized.mapNodes);
  } finally {
    if (priorFixtureMode === undefined) delete process.env.WORKSHOPLM_SEEDED_FIXTURE;
    else process.env.WORKSHOPLM_SEEDED_FIXTURE = priorFixtureMode;
    await rm(root, { recursive: true, force: true });
...
it("promotes the proposed decision instead of prose that merely mentions recommendations", async () => {
  ...
  try {
    await ingestSource({
      title: "Launch notes",
      origin: "Meeting",
      text: "We need a client-ready launch plan that keeps every recommendation tied to the meeting notes. The team recommends a two-week pilot, confirmed ownership, and a visible leadership decision path. Every recommendation must remain traceable to its Source.",
    }, root);
    const organized = organizeGroundedMap(root);
    ...
    expect(organized.mapNodes.find((node) => node.id === "map-direction")).toMatchObject({
      title: "A two-week pilot, confirmed ownership, and a visible…",
      body: "The team recommends a two-week pilot, confirmed ownership, and a visible leadership decision path",
    });
    ...
    expect(approved.frame?.markdown).toContain("## Direction\nThe team recommends a two-week pilot, confirmed ownership, and a visible leadership decision path");
    expect(approved.frame?.markdown.match(/The team recommends a two-week pilot/g)).toHaveLength(1);
    lockManualStyle({}, root);
    const withPresentation = await generateOutput("deck", root);
    const presentation = await readFile(join(root, withPresentation.outputs.at(-1)!.relativePath), "utf8");
    const presentationRecommendation = presentation.match(/<section class="slide recommendation[\s\S]*?<\/section>/)?.[0];
    expect(presentationRecommendation?.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ")).toContain("The team recommends a two-week pilot, confirmed ownership, and a visible leadership decision path");
    expect(presentationRecommendation).not.toContain("Every recommendation must remain traceable");
    const withInfographic = await generateOutput("infographic", root);
    const infographic = await readFile(join(root, withInfographic.outputs.at(-1)!.relativePath), "utf8");
    const infographicRecommendation = infographic.match(/<article class="infographic-card" data-layout="recommendation"[\s\S]*?<\/article>/)?.[0];
    expect(infographicRecommendation?.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ")).toContain("The team recommends a two-week pilot, confirmed ownership, and a visible leadership decision path");
    ...
    const storyboard = generateStoryboard(root).storyboard;
    expect(storyboard.panels.at(-1)?.narration).toBe("The team recommends a two-week pilot, confirmed ownership, and a visible leadership decision path.");
    expect(storyboard.panels.at(-1)?.title).toBe("The direction");
    expect(new Set(storyboard.panels.map((panel) => panel.title)).size).toBe(storyboard.panels.length);
  } finally {
    if (priorFixtureMode === undefined) delete process.env.WORKSHOPLM_SEEDED_FIXTURE;
    else process.env.WORKSHOPLM_SEEDED_FIXTURE = priorFixtureMode;
    await rm(root, { recursive: true, force: true });
...
it("persists a grounded Conversation without turning a question into a Source", async () => {
  ...
  expect(state.conversationTurns[1]?.text).toContain("Evidence becomes an editable knowledge system");
...
it("refreshes the weekly Map and Brief after a new meeting while preserving the reused Style and exact deck trace", async () => {
  ...
  expect(() => applyWorkshopAction("approveBrief", root)).toThrow(/Update the Map/);
...
it("turns spoken transformation and process language into complete professional slide copy", async () => {
  ...
  await ingestSource({ title: "Founder brainstorm", origin: "Founder-provided recording", text: "WorkshopLM should show how a messy spoken idea becomes a grounded Map, an approved Brief, coherent visuals, an editable Storyboard, and the final demo Video. Judges need one continuous Capture to Map to Brief to Create path. Every factual claim must retain a visible source locator. The public demo Video must remain under three minutes." }, root);
  applyWorkshopAction("approveBrief", root);
  lockManualStyle({}, root);
  const generated = await generateOutput("deck", root);
  ...
  expect(html).toContain('<div class="sequence-step"><span>Map</span><small>See evidence, synthesis, and direction.</small></div>');
  expect(html).toContain('<div class="sequence-step"><span>Brief</span><small>Approve what should be created.</small></div>');
...
it("generates a source-grounded narrative Storyboard instead of sequencing output formats", async () => { const root = await mkdtemp(join(tmpdir(), "workshop-generated-story-")); const ingested = await ingestSource({ title: "Evidence", origin: "Fixture", text: "A strong opportunity starts with clear knowledge. The current work is fragmented across handoffs. Source evidence remains visible. The experience should feel immediate for a professional. Every change must preserve trust and traceability." }, root); const sourceId = ingested.sourceItems.at(-1)!.id; setActiveSourceScope([sourceId], root); organizeGroundedMap(root); applyWorkshopAction("approveBrief", root); lockManualStyle({}, root); generateAssetPlan(root); const state = generateStoryboard(root); expect(state.storyboard).toMatchObject({ stale: false }); expect(state.storyboard.panels).toHaveLength(5); const titles = state.storyboard.panels.map((panel) => panel.title); expect(new Set(titles).size).toBe(5); expect(titles).toEqual(expect.arrayContaining(["The friction", "The evidence", "The experience", "Trust by design"])); expect(state.storyboard.panels.map((panel) => panel.narration).join(" ")).toContain("The current work is fragmented across handoffs."); expect(titles.join(" ")).not.toMatch(/Presentation|Infographic|Image set|Demo video/); expect(state.storyboard.panels[0]?.narration).not.toContain("Fixture · chunk 01"); expect(state.storyboard.panels.every((panel) => panel.durationSeconds >= Math.ceil(panel.narration.split(/\s+/).length / 2.5) + 1)).toBe(true); expect(state.storyboard.panels[0]).toMatchObject({ claimIds: [expect.stringMatching(/^claim-/)], evidence: [{ claimId: expect.stringMatching(/^claim-/), sourceId: expect.stringMatching(/^source-/), chunkId: expect.stringMatching(/^chunk-/), locator: "Fixture · chunk 01" }] }); expect(() => assertStoryboardGrounding({ ...state, storyboard: { ...state.storyboard, panels: state.storyboard.panels.map((panel, index) => index ? panel : { ...panel, claimIds: ["claim-tampered"] }) } })).toThrow(/claim IDs do not match/); expect(state.storyboardApproved).toBe(false); await rm(root, { recursive: true, force: true }); });
...
it("creates a grounded Audio Overview and preserves source edges through an edited version", async () => {
  ...
  expect(first.sections.every((section) => {
    const reference = section.evidence[0];
    const claim = generated.claims.find((candidate) => candidate.id === reference?.claimId);
    return section.evidence.length === 1 && Boolean(reference?.sourceId && reference.chunkId && reference.locator && claim && claim.sourceId === reference.sourceId && claim.chunkId === reference.chunkId && claim.locator === reference.locator);
  })).toBe(true);
