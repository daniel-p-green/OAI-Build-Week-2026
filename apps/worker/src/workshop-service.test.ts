...
it("analyzes a public website and locks only the reviewed Style values", async () => {
  ...
  expect(JSON.parse(await readFile(join(root, state.designArtifact!.tokensPath), "utf8"))).toMatchObject({ schemaVersion: 3, source: "website", palette: { accent: "#335577" }, paletteRoles: { accent: { value: "#335577", source: "website" } }, typographyRoles: { heading: { family: "Studio Sans", availability: "unverified" } }, brandAssets: [{ sha256: state.style!.brandAssets[0]!.sha256 }], licensedFonts: [], intentProfile: "board_deck" });
  applyWorkshopAction("approveBrief", root);
...
it("rebinds a regenerated image to the Storyboard and still requires the professional to approve it", async () => { const root = await mkdtemp(join(tmpdir(), "workshop-story-images-")); await ingestSource({ title: "Evidence", origin: "Fixture", text: "The approved video must use the images reviewed in its storyboard." }, root); applyWorkshopAction("approveBrief", root); lockManualStyle({}, root); generateAssetPlan(root); createImageBatch(root); const generated = generateStoryboard(root); expect(generated.storyboard.panels.map((panel) => [panel.imagePanelId, panel.imagePanelVersion])).toEqual([["image-panel-1", 1], ["image-panel-2", 1], ["image-panel-3", 1], ["image-panel-4", 1], ["image-panel-5", 1]]); expect(applyWorkshopAction("approveStoryboard", root).storyboardApproved).toBe(true); const changed = selectImagePanelForRegeneration("image-panel-2", root); expect(changed.storyboardApproved).toBe(false); expect(changed.storyboard).toMatchObject({ stale: true }); expect(changed.storyboard.panels[1]).toMatchObject({ stale: true, approved: false, imagePanelVersion: 1 }); expect(changed.imageBatch?.panels[1]).toMatchObject({ version: 2, state: "selected_for_regeneration" }); expect(() => applyWorkshopAction("approveStoryboard", root)).toThrow(/current approved panels/); const rebound = recordGeneratedImagePanel("image-panel-2", { relativePath: "generated/images/image-panel-2-v2.png", sha256: "a".repeat(64), provenance: { model: "gpt-image-2", size: "1024x1024", quality: "medium", referenceId: changed.imageBatch!.referenceId, generatedAt: new Date().toISOString() } }, root); expect(rebound.storyboardApproved).toBe(false); expect(rebound.storyboard).toMatchObject({ stale: false }); expect(rebound.storyboard.panels[1]).toMatchObject({ stale: false, approved: true, imagePanelVersion: 2 }); expect(applyWorkshopAction("approveStoryboard", root).storyboardApproved).toBe(true); await rm(root, { recursive: true, force: true }); });
