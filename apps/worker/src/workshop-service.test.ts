...
it("analyzes a public website and locks only the reviewed Style values", async () => {
  ...
  expect(JSON.parse(await readFile(join(root, state.designArtifact!.tokensPath), "utf8"))).toMatchObject({ schemaVersion: 2, source: "website", palette: { accent: "#335577" }, paletteRoles: { accent: { value: "#335577", source: "website" } }, typographyRoles: { heading: { family: "Studio Sans", availability: "unverified" } }, licensedFonts: [], intentProfile: "board_deck" });
  const confirmed = await lockWebsiteStyle("https://example.com/style", root, fetchImpl as typeof fetch, "board_deck", { name: suggestion.name, accent: "#335577", ink: suggestion.ink, paper: suggestion.paper, headingFont: "Studio Sans", bodyFont: "Studio Sans", fontsConfirmed: true, logos: suggestion.logos, references: suggestion.references, negativeRules: ["No gradients"], intentProfile: "board_deck" });
