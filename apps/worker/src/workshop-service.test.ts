...
it("creates and approves a versioned, source-traced Sketch artifact, then stales it", async () => { const root = await mkdtemp(join(tmpdir(), "workshop-sketch-")); applyWorkshopAction("approveBrief", root); const sketch = createSketch(root).sketch; expect(sketch).toMatchObject({ version: 1, graphRevision: 0, relativePath: "generated/sketch-v1.svg", sha256: expect.stringMatching(/^[a-f0-9]{64}$/), stale: false, approved: false }); expect(sketch?.nodes).toHaveLength(4); expect(sketch?.claimIds.length).toBeGreaterThan(0); expect(await readFile(join(root, sketch!.relativePath), "utf8")).toContain("hand-drawn Sketch"); expect(resolveWorkshopArtifact("sketch", root)).toMatchObject({ contentType: "image/svg+xml; charset=utf-8" }); expect(approveSketch(root).sketch?.approved).toBe(true); expect(applyMapOperation({ type: "update_node", nodeId: "node-promise", patch: { label: "Changed" } }, root).sketch).toMatchObject({ stale: true, approved: false }); await rm(root, { recursive: true, force: true }); });
