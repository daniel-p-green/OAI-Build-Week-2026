...
it("generates a source-grounded narrative Storyboard instead of sequencing output formats", async () => { const root = await mkdtemp(join(tmpdir(), "workshop-generated-story-")); await ingestSource({ title: "Evidence", origin: "Fixture", text: "A strong opportunity starts with clear knowledge. The current work is fragmented across handoffs. Source evidence remains visible. The experience should feel immediate for a professional. Every change must preserve trust and traceability." }, root); applyWorkshopAction("approveBrief", root); lockManualStyle({}, root); generateAssetPlan(root); const state = generateStoryboard(root); expect(state.storyboard).toMatchObject({ stale: false }); expect(state.storyboard.panels).toHaveLength(5); expect(state.storyboard.panels.map((panel) => panel.title)).toEqual(["The opportunity", "The friction", "The evidence", "The experience", "Trust by design"]); expect(state.storyboard.panels.map((panel) => panel.narration).join(" ")).toContain("The current work is fragmented across handoffs."); expect(state.storyboard.panels.map((panel) => panel.title).join(" ")).not.toMatch(/Presentation|Infographic|Image set|Demo video/); expect(state.storyboard.panels[0]?.narration).not.toContain("Fixture · chunk 01"); expect(state.storyboard.panels.every((panel) => panel.durationSeconds >= Math.ceil(panel.narration.split(/\s+/).length / 2.5) + 1)).toBe(true); expect(state.storyboard.panels[0]).toMatchObject({ claimIds: [expect.stringMatching(/^claim-/)], evidence: [{ claimId: expect.stringMatching(/^claim-/), sourceId: expect.stringMatching(/^source-/), chunkId: expect.stringMatching(/^chunk-/), locator: "Fixture · chunk 01" }] }); expect(() => assertStoryboardGrounding({ ...state, storyboard: { ...state.storyboard, panels: state.storyboard.panels.map((panel, index) => index ? panel : { ...panel, claimIds: ["claim-tampered"] }) } })).toThrow(/claim IDs do not match/); expect(state.storyboardApproved).toBe(false); await rm(root, { recursive: true, force: true }); });
