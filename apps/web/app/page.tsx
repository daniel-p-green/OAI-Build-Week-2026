  ...
  return (
    <main className="workspace">
      ...
      <section className="center" aria-label="Workshop canvas">
        ...
        </section> : view === "Brief" ? <Brief approved={persisted?.briefApproved ?? briefApproved} frame={persisted?.frame} /> : view === "Story" ? <Storyboard approved={persisted?.storyboardApproved ?? storyApproved} ready={Boolean(persisted?.style && !persisted.style.stale)} onApprove={() => { void mutate("approveStoryboard").then(() => setStoryApproved(true)); }} /> : view === "Design" ? <DesignReview style={persisted?.style} onLock={() => { void mutate("lockManualStyle"); }} /> : <Trace />}
      </section>
...
function Storyboard({ approved, ready, onApprove }: { approved: boolean; ready: boolean; onApprove: () => void }) { return <section className="storyboard"><div className="filmstrip">{["Raw thought", "Cited Map", "Approved brief", "Finished work"].map((label, index) => <button key={label} className={index === 1 ? "frame selected-frame" : "frame"}><span>{index + 1}</span><strong>{label}</strong><small>0:0{index + 3}</small></button>)}</div><article className="panel-editor"><span className="eyebrow">PANEL 2 · GROUNDED MAP</span><h1>Messy turns settle into cited clusters.</h1><p>Each card shows an evidence state and an inspectable source locator. Narration is AI-generated and disclosed.</p><div><span className="grounded-tag">Grounded · 2 sources</span><span>Duration 0:05</span></div></article><button className="approve render-approve" disabled={approved || !ready} onClick={onApprove}>{approved ? "Storyboard approved" : ready ? "Approve storyboard & render" : "Lock style first"}</button></section>; }
