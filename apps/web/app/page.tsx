...
function DesignReview({ style, onLock, onWebsiteLock }: { style?: PersistedWorkshop["style"]; onLock: () => void; onWebsiteLock: (url: string) => Promise<void> }) { const [websiteUrl, setWebsiteUrl] = useState(""); const [websiteStatus, setWebsiteStatus] = useState<string | null>(null); async function captureWebsite() { try { setWebsiteStatus("Inspecting public site…"); await onWebsiteLock(websiteUrl); setWebsiteStatus("Website tokens locked."); } catch (error) { setWebsiteStatus(error instanceof Error ? error.message : "Website style capture failed"); } } return <section className="document design-doc"><div className="document-copy"><span className="eyebrow">STYLE LIBRARY</span><h1>DESIGN.md</h1><div className="swatches"><i /><i /><i /><i /></div><h3>{style?.name ?? "Manual Brand Foundation"}</h3><p>Capture inspectable tokens from a safe public website, or lock the manual baseline before approving the storyboard.</p><label className="website-style">Public website URL<input type="url" value={websiteUrl} onChange={(event) => setWebsiteUrl(event.target.value)} placeholder="https://example.com" /></label>{websiteStatus && <small className="capture-status">{websiteStatus}</small>}<button className="outline-action" disabled={!websiteUrl} onClick={() => { void captureWebsite(); }}>Capture website style</button></div><aside><span className={style && !style.stale ? "approved-label" : "pending-label"}>{style && !style.stale ? `Style v${style.version} locked` : "Style not locked"}</span><dl><dt>Brand foundation</dt><dd>{style?.name ?? "Manual tokens"}</dd><dt>Intent</dt><dd>Judge-facing demo</dd></dl><button className="approve" onClick={onLock}>Lock manual style</button></aside></section>; }
