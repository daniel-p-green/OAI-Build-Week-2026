...
type PersistedWorkshop = { briefApproved: boolean; storyboardApproved: boolean; videoState: "blocked" | "queued" | "rendering" | "rendered"; firstTranscriptAt?: string; firstRenderedOutputAt?: string; sourceItems: SourceItem[]; sourceChunks?: { id: string; sourceId: string; text: string; locator: string }[]; claims?: { id: string; sourceId: string; chunkId: string; text: string; locator: string }[]; candidates?: { id: string; category: "goal" | "audience" | "claim" | "constraint" | "question"; text: string; sourceId: string; chunkId: string; locator: string }[]; mapNodes: MapNode[]; mapEdges?: { id: string; from: string; to: string; kind: string; label?: string }[]; assetPlan?: { version: number; graphRevision: number; briefVersion: number; styleVersion: number; visualDnaVersion?: number; evidenceClaimIds: string[]; items: { id: string; outputType: string; title: string; prompt: string; locator: string }[]; stale: boolean }; storyboard: { version: number; stale: boolean; panels: { id: string; title: string; narration: string; durationSeconds: number; approved: boolean; stale: boolean }[] }; imageBatch?: { id: string; stale: boolean; referenceId: string; panels: { id: string; version: number; prompt: string; state: "planned" | "selected_for_regeneration" }[] }; outputs: { id: string; type: "deck" | "infographic"; stale: boolean; artifactPath: string }[]; frame?: { version: number; markdown: string; markdownPath: string; executablePath: string; stale: boolean }; style?: { version: number; name: string; accent: string; ink: string; paper: string; logos: string[]; licensedFonts: string[]; references: string[]; negativeRules: string[]; intentProfile: "client_facing_pitch" | "board_deck" | "internal_workshop"; stale: boolean }; designArtifact?: { version: number; styleVersion: number; markdownPath: string; tokensPath: string; stale: boolean }; visualDna?: { version: number; styleVersion: number; palette: { accent: string; ink: string; paper: string }; compositionRules: string[]; textureRules: string[]; imageRules: string[]; negativeRules: string[]; approved: boolean; stale: boolean } };
...
export default function WorkshopPage() {
  ...
  async function visualDna(action: "createVisualDna" | "approveVisualDna") { const response = await fetch("/api/workshop", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action }) }); const state = await response.json() as PersistedWorkshop & { error?: string }; if (!response.ok) throw new Error(state.error ?? "Visual DNA action failed"); setPersisted(state); }
  async function generateAssetPlan() { const response = await fetch("/api/workshop", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "generateAssetPlan" }) }); const state = await response.json() as PersistedWorkshop & { error?: string }; if (!response.ok) throw new Error(state.error ?? "Asset planning failed"); setPersisted(state); }
  ...
  return (
    <main className="workspace">
      ...
      <aside className="studio" aria-label="Studio">
        ...
        <section className="output-actions"><span>Output types</span><div><button onClick={() => { void generateAssetPlan(); }}>Asset plan</button><button onClick={() => { void generateOutput("deck"); }}>Deck</button><button onClick={() => { void generateOutput("infographic"); }}>Infographic</button><button onClick={() => { void createImageBatch(); }}>Images</button><button onClick={() => setView("Story")}>Storyboard</button><button disabled={!storyApproved} onClick={renderVideo}>Video</button></div></section>
        ...
        <section className="history"><h3>Output history</h3><button>FRAME.md <small>{persisted?.frame?.stale ? "stale" : persisted?.frame ? `v${persisted.frame.version} current` : "awaiting map approval"}</small></button><button>DESIGN.md <small>{persisted?.style ? `v${persisted.style.version} locked` : "awaiting lock"}</small></button>{persisted?.assetPlan && <div className="asset-plan"><span>Asset plan <small>{persisted.assetPlan.stale ? "stale" : `v${persisted.assetPlan.version} · ${persisted.assetPlan.items.length} outputs`}</small></span>{persisted.assetPlan.items.map((item) => <small key={item.id} title={item.prompt}>{item.outputType} · {item.title}</small>)}</div>}{persisted?.imageBatch && <div className="image-batch"><span>Image batch <small>{persisted.imageBatch.stale ? "stale" : `${persisted.imageBatch.panels.length} panels · ${persisted.imageBatch.referenceId}`}</small></span>{persisted.imageBatch.panels.map((panel) => <button key={panel.id} disabled={persisted.imageBatch?.stale} title={panel.prompt} onClick={() => { void regenerateImagePanel(panel.id); }}><span>{panel.id.replace("image-panel-", "Panel ")} · v{panel.version}</span><small>{panel.state === "selected_for_regeneration" ? "selected" : "Regenerate"}</small></button>)}</div>}{(persisted?.outputs ?? []).map((output) => <button key={output.id}>{output.type} <small>{output.stale ? "stale" : output.artifactPath}</small></button>)}</section>
      </aside>
...
function Brief({ approved, frame }: { approved: boolean; frame?: PersistedWorkshop["frame"] }) { return <section className="document"><div className="document-copy"><span className="eyebrow">EXECUTABLE BRIEF</span><h1>FRAME.md</h1><p className="lede">{frame?.markdown.split("\n").find((line) => line.startsWith("## Outcome")) ? frame.markdown.split("\n")[3] : "Approve the current Map to generate this source-grounded brief."}</p><pre className="frame-markdown">{frame?.markdown ?? "Preview only — no approved Map version yet."}</pre></div><aside><span className={approved && !frame?.stale ? "approved-label" : "pending-label"}>{approved && !frame?.stale ? `Map v${frame?.version ?? 1} approved` : frame?.stale ? "Map changed — stale" : "Preview only"}</span><dl><dt>Dependency</dt><dd>Current Map</dd><dt>Version</dt><dd>{frame ? `FRAME v${frame.version}` : "—"}</dd><dt>Artifacts</dt><dd>{frame ? `${frame.markdownPath} · ${frame.executablePath}` : "—"}</dd></dl></aside></section>; }
