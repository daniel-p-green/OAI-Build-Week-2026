...
describe("official Apps in ChatGPT UI implementation", () => {
  ...
  it("keeps internal product language out of visible copy", () => {
    ...
    for (const required of ["Approve brief", "Choose style", "Use this style", "Update style", "Create outputs", "Update outputs", "View outputs", "Show source", "Show on map", "Images not created yet", "Review before video", "Create video", "Up to date", "Needs update"]) expect(page).toContain(required);
