...
| --- | --- | --- |
| `createGroundedResponse` | Responses + GPT-5.6 routing (`terra` default, `sol` challenger) | synthesis over locally retrieved evidence bundles, questions, typed plan proposals |
| `extractWorkshopGraph` | Responses + GPT-5.6 routing (`sol` default, `terra` challenger) | structured claims, nodes, edges, and unresolved items |
