...
Founder mode does not inject the authorized sample transcript or sanitized operator fixtures. Its private review uses a deterministic Evidence → Synthesis → Direction Map, while the paid command replaces that Map with the GPT-5.6 Terra result. After locking Style, preflight creates the grounded Sketch before evaluating Create completeness, so all required pre-Video work is present for review. Every preflight output and planned media evidence edge must remain grounded in the founder Source. The Source sheet shows `Private` or `Shareable` directly so the second command is an explicit sharing decision rather than an invisible metadata change.
