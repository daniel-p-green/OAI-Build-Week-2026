...
export const StoryboardPanel = z.object({ id: z.string().min(1), purpose: z.string().min(1), claimIds: z.array(ClaimId), voiceover: z.string().min(1), onScreenText: z.string(), durationSeconds: z.number().positive(), composition: z.string().min(1), visualDnaVersionId: VersionId, imagePanelId: z.string().min(1).optional(), imagePanelVersion: z.number().int().positive().optional(), transition: z.string().min(1), approved: z.boolean(), staleState: StaleState }).strict().superRefine((panel, context) => { if (Boolean(panel.imagePanelId) !== Boolean(panel.imagePanelVersion)) context.addIssue({ code: z.ZodIssueCode.custom, message: "storyboard image bindings require both panel ID and version" }); });
