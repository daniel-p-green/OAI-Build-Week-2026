...
for (const model of models) {
  for (const testCase of routingBenchmarkCases) {
    ...
    const outputText = payload.output_text ?? null;
    results.push({ model, case: testCase.id, operation: testCase.operation, status: response.status, latencyMs: Math.round(performance.now() - startedAt), responseModel: payload.model ?? null, usage: payload.usage ?? null, outputText, quality: scoreBenchmarkOutput(testCase, outputText), error: payload.error ?? null });
  }
}
