...
const plannedRequests = models.length * routingBenchmarkCases.length;
const requestCeiling = requireBenchmarkRequestCeiling(process.env.WORKSHOPLM_MAX_PAID_REQUESTS, plannedRequests);
let usedRequests = 0;
const results = [];
for (const model of models) {
  for (const testCase of routingBenchmarkCases) {
    if (usedRequests >= requestCeiling) throw new Error(`Provider request ceiling reached (${requestCeiling}). Refusing another paid request.`);
    usedRequests += 1;
...
const artifact = { generatedAt: new Date().toISOString(), task: "gpt56-routing-benchmark", methodology: { models, cases: routingBenchmarkCases.map(({ id, operation, reasoningEffort, maxOutputTokens }) => ({ id, operation, reasoningEffort, maxOutputTokens })), measures: ["HTTP status", "end-to-end latencyMs", "reported usage", "deterministic JSON/evidence score"], requestBudget: { used: usedRequests, ceiling: requestCeiling }, pricing: "No dollar estimate is inferred; compare usage against the current official model pricing at decision time." }, results };
