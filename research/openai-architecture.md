...
| Narration and Audio Overview | Audio Speech API with `gpt-4o-mini-tts` | render approved Storyboard voiceover in bounded scene segments and a separately grounded, reviewed Audio Overview; use the live-verified `cedar` voice for the demo | Disclose clearly that the voice is AI-generated. Store model, voice, instructions, version, duration, file hash, and exact source coverage with every audio artifact. |
