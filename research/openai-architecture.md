...
| Conversation and voice | Codex desktop/CLI task through the unified plugin, plus WorkshopLM's capture-only Realtime control | text clarification and typed Workshop commands in Codex; voice transcript capture inside the Workshop | WorkshopLM does not duplicate chat or store ChatGPT tokens. The installed classic ChatGPT app exposes no unified local-plugin surface, so Work parity and native task/voice synchronization are not claimed. |
