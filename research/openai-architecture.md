...
| --- | --- | --- | --- |
| Workshop reasoning and structured operations | Responses API with `gpt-5.6-sol` | source synthesis, claim extraction, graph operations, `FRAME.md`, asset plans, storyboard structures, propagation previews, and model-based evaluations | Require structured schemas for executable outputs; never parse free-form prose into domain state ad hoc. |
...
| Conversational image revision | Responses API with `gpt-5.6-sol` and the `image_generation` tool | multi-turn edits when the user refines a selected image conversationally | Use this path for iterative edits, not as the only batch-production mechanism; the tool manages its own GPT Image model selection. |
