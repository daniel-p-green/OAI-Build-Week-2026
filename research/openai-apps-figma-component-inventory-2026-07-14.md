...
| `StyleSetup` | SideSheet + ListGroup/ListRow + ListRowAction + Input/TextArea + Button + palette/type domain previews | Website/manual source and Intent Profile use existing selectable rows and fields. Manual brand details are secondary disclosure; no tabs, custom radio family, or third approval gate. |
...
| `Workbench` | Full screen modal `2133:27199` + Navigation/Header `2:626` + Sidebar desktop header `2123:26990` | Stable three-region layout only. Sources and Production use official white columns and dividers; the current object remains the flexible full-screen content region. |
