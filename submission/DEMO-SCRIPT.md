...
| --- | --- | --- |
| 0:00–0:12 | Show WorkshopLM Map at desktop width. | “WorkshopLM turns raw professional thinking into finished, source-traceable work.” |
...
| 1:15–1:28 | Show the Style summary with palette, type, layout, and negative rules. | “Style is reviewable and versioned, so every Output follows the same visual rules.” |
| 1:28–1:47 | Open Outputs. Show the presentation and infographic previews, then the image set. If a logged live image set exists, show it; otherwise keep `Planned` visible and say `planned`. | “One approved state produces source-traceable, branded Outputs. This recorded path plans the image set without pretending provider pixels exist.” |
...
| 2:05–2:23 | Choose `Create video`, show the MP4, open a focused Output, and choose `Show source` on one factual claim. | “The local worker renders the approved current Storyboard. Show source follows the Output back through the Brief and Map to the exact evidence.” |
...
- Show exactly two approvals: Map as brief, then storyboard.
