...
| Unified Codex plugin | “The Codex desktop app activated the packaged `$workshoplm` skill, used its read-only MCP tools to search and fetch exact grounded evidence, and rendered the local Workshop doorway.” | `artifacts/spikes/plugin-desktop-host-2026-07-15.json`; `artifacts/spikes/plugin-skill-activation-2026-07-15.json` | This proves Codex desktop and CLI read paths, not ChatGPT Work, plugin writes, or a `/feedback` Session ID. |
...
| Final self-produced submission | Provider-backed final Devpost copy, README narrative, deck, thumbnails, Storyboard, narration, and video in one verified Output set | Describe the current 17-asset set as `partial` and recorded-fixture based. |
