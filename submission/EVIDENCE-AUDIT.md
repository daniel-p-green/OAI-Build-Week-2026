...
| Recorded submission Output set | A 17-asset set includes editable presentation and infographic PowerPoints, source locators, hashes, an input fingerprint, per-scene Video provenance, and immutable HTML/JSON build records; its manifest is explicitly `partial` because provider images, narration, and final public video are absent. | Proven partial |
