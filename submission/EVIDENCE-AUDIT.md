...
| Plugin write-to-service boundary | An isolated plugin run imported and grounded a real local source, approved exact `map-r1`, created a deck, image plan, and `storyboard-v2`, approved that exact Storyboard, queued a real `render_video` job, and fetched four deck claim→chunk→source edges. Stale version tests dispatch no service mutation. | Proven locally; Codex is the supported host |
...
| Recorded submission Output set | An 18-asset set includes editable presentation and infographic PowerPoints, a grounded Audio Overview script, source locators, hashes, an input fingerprint, per-scene Video provenance, and immutable HTML/JSON build records; its manifest is explicitly `partial` because provider voice, GPT-5.6 reasoning, images, Video narration, Audio Overview speech, and the final public video are absent. | Proven partial |
...
WorkshopLM is **not complete**. The local deterministic product, editable Map, versioned Outputs, weekly refresh path, Codex desktop plugin read/browser path, and evidence chain are substantial and verified. A real professional's cold `Send` or `Revise` decision, provider-backed GPT-5.6 reasoning, image bytes, narration, one live Realtime microphone turn, uncoached empty-Workshop use, the final public video, and submitted links remain open under `GOAL.md`. ChatGPT Work parity is outside the verified host boundary rather than an unfinished product claim.
