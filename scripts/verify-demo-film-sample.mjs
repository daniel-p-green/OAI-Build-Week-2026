...
async function main() {
  ...
  assert(manifest.compositor.jitterProneZoompan === false && manifest.compositor.spatialTransforms === false && manifest.compositor.transition === "neutral-dip", "The sample film has not removed jitter-prone spatial motion or its neutral transition treatment.");
  ...
  assert(opening?.openingProof?.type === "finished-work-to-map" && opening.openingProof.durationSeconds >= 3 && opening.openingProof.durationSeconds <= 6 && opening.openingProof.transition === "neutral-dip", "The sample film must open on verified finished work before revealing the Map.");
  ...
  assert(!manifest.shots.some((shot) => shot.editorialCue), "The clean film must not add persistent editorial callouts over product interaction.");
